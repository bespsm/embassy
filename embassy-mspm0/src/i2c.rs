...
impl<'d> I2c<'d, Blocking> {
    ...
    fn master_blocking_continue(&mut self, length: usize, send_ack_nack: bool, send_stop: bool) -> Result<(), Error> {
        ...
        spin_until(BUSY_IDLE_SPIN_LIMIT, || !self.info.regs.controller(0).csr().read().busy())?;
    ...
    fn master_blocking_read(
        ...
        if !restart {
            spin_until(BUSY_IDLE_SPIN_LIMIT, || self.info.regs.controller(0).csr().read().idle())?;
        }
    ...
    pub fn blocking_read(&mut self, address: u8, read: &mut [u8]) -> Result<(), Error> {
        ...
        spin_until(BUSY_IDLE_SPIN_LIMIT, || !self.info.regs.controller(0).csr().read().busbsy())?;
